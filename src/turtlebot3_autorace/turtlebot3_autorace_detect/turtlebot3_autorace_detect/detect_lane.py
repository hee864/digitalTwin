...
class DetectLane(Node):
    ...
    def __init__(self):
        ...
        #추가: 왼쪽 차선과 오른쪽 차선의 두개 다  퍼블리시
 
        self.pub_lane_bounds = self.create_publisher(Float64MultiArray, '/lane/x_bounds', 10)
        #r(Float64MultiArray, '/lane/x_bounds', 10) 컨트롤 라인에 추가
    ...
    def cbFindLane(self, image_msg):
        ...
        # self.get_logger().info(f"카메라 왼쪽 끝: x = 0, 오른쪽 끝: x = {image_width - 1}")
        # 전처리 추가
    ...
    def fit_from_lines(self, lane_fit, image):
        ...
        middle_idx = len(ploty) // 2  # 화면 세로 중앙 위치 (예: y=240)

        #이 부분 추가
        if hasattr(self, 'left_fitx') and hasattr(self, 'right_fitx'):
            if len(self.left_fitx) > middle_idx and len(self.right_fitx) > middle_idx:
                left_x = float(self.left_fitx[middle_idx])
                right_x = float(self.right_fitx[middle_idx])

                bounds_msg = Float64MultiArray()
                bounds_msg.data = [left_x, right_x]
                self.pub_lane_bounds.publish(bounds_msg)

                # self.get_logger().info(f" lane bounds: left_x={left_x:.1f}, right_x={right_x:.1f}")
